...
class TS_Class:
    ...
    def get_cycl_adj(self, var = None, weekdays = False, ext_fit = False, fit = None, return_fit = False, filter_zeros = True, filter_rain = True, rain_quantile = 0.9):
        ...
        if not ext_fit:
            self.data = cyclical_adj_full(self.data, weekdays, var, return_fit, filter_zeros, filter_rain, rain_quantile)
        else:
...
##    rain_prev = rainy.shift(list(range(1, 1+num_back))).any(axis = 1) ##Check whether any of previous num_back obs. were classified as rainy
    shifted_rainy = pd.DataFrame()
    for i in range(1, 1+num_back):
        shifted_rainy[f'rainy_shift_{i}'] = rainy.shift(i)
    rain_prev = shifted_rainy.any(axis = 1) ##Check whether any of previous num_back obs. were classified as rainy
